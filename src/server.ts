...
export async function bootstrap() {
  ...
  await orchestrator.startInsightListener((sessionId, insight) => {
    console.info(`Sending insight to session ${sessionId}:`, insight.content);
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('start-call', async (data: { sessionId?: string; title: string }) => {
      try {
        ...
        console.info(`Call session created in DB: ${sessionId} - ${title}`);
    ...
    socket.on('feedback', async (data: { sessionId: string; id: string; status: string }) => {
      try {
        ...
        console.info(`Feedback persisted for ${data.id}: ${status}`);
      } catch (error) {
    ...
    socket.on('end-call', async (data: { sessionId: string }) => {
      try {
        ...
        console.info(`Call session ${sessionId} ended and persisted.`);
      } catch (error) {
  ...
  httpServer.listen(PORT, () => {
    console.info(`Wingman Backend running on port ${PORT}`);
  });
}
