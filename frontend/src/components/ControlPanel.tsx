      ...
      <div className="bg-[var(--bg-panel)] border border-[var(--border-industrial)] p-4 flex flex-col shrink-0">
        ...
        {!isCalling ? (
          ...
          >
            <div className="flex items-center gap-3">
              <Mic
                size={18}
                className="text-[var(--text-muted)] group-hover:text-[var(--accent-live)]"
              />
              <span className="font-display font-bold uppercase tracking-wider text-sm">
                Initialize Sec-Link
              </span>
            </div>
            <span className="text-[var(--text-muted)] text-xs group-hover:text-[var(--accent-live)] font-bold">
              INIT
            </span>
          </button>
        ) : (
          ...
          >
            <div className="flex items-center gap-3">
              <PhoneOff size={18} />
              <span className="font-display font-bold uppercase tracking-wider text-sm flex-1 text-left">
                Terminate Link
              </span>
            </div>
            <span className="font-bold text-[10px] border border-current px-1 hidden md:block uppercase tracking-widest">
              Halt
            </span>
          </button>
        )}
        ...
        <div className="mt-6 flex flex-col gap-2">
          <div className="flex justify-between items-end">
            <span className="text-[10px] text-[var(--text-muted)] uppercase tracking-widest border-b border-[var(--border-industrial)] flex-1 pb-1 mr-2">
              Audio Stream
            </span>
            <span
              className={`text-xs ${isCalling ? 'text-[var(--accent-live)]' : 'text-[var(--text-muted)]'}`}
            >
          ...
          <div className="flex justify-between items-end">
            ...
            <span
              className={`text-xs ${socketConnected ? 'text-[var(--accent-live)]' : 'text-[var(--accent-alert)]'}`}
            >
